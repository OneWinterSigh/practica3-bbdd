...
public class Trabaja extends DBTable {
	...
	boolean createTable() {
		if (!this.conn.tableExists("Trabaja")) {
			String query = "CREATE TABLE Trabaja (id_empleado int not null, codigo_local int not null, "
					+ "fecha_inicio date not null, fecha_fin date, PRIMARY KEY (fecha_inicio) "
	...
	boolean insertEntry() {
		try {
			if (conn.query("SELECT * FROM Trabaja WHERE id_empleado = " + id_empleado).next()) {
				if (conn.query("SELECT * FROM Trabaja WHERE codigo_local = " + id_local).next()) {
	...
	boolean updateEntry() {
		if (!this.conn.tableExists("TRABAJA")) {
			String query = "UPDATE Trabaja SET id_empleado= " + id_empleado
					+ ", codigo_local=" + id_local
					+ ", fecha_inicio=" + fecha_inicio
					+ ", fecha_fin=" + fecha_fin
					+ " WHERE id_empleado= " + id_empleado
					+ " AND codigo_local= " + id_local
	...
	boolean deleteEntry() {
		...
		if (this.conn.tableExists("TRABAJA")) {
			String query = "DELETE FROM TRABAJA WHERE id_empleado = " + id_empleado + " AND id_local=" + id_local
					+ "AND fecha_inicio=" + fecha_inicio;
	...
	void getEntryChanges() {
		...
		if (this.conn.tableExists("TRABAJA")) {
			try {
				String query = "SELECT * FROM TRABAJA WHERE id_empleado = " + id_empleado + " AND id_local=" + id_local
						+ "AND fecha_inicio=" + fecha_inicio;
				rs = conn.query(query);
